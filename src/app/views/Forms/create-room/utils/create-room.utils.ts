import type { RoomCreatePayload, RoomCreateErrors } from "../types/create-room.type";
...
export function uniqueNumbers(arr: number[]): number[] {
...
/** Angular Validators */
export function urlValidator(ctrl: AbstractControl): ValidationErrors | null {
  const v = String(ctrl.value ?? "").trim();
  if (!v) return { required: true };
  if (!isValidUrl(v)) return { url: true };
  return null;
}

export function latValidator(ctrl: AbstractControl): ValidationErrors | null {
  const n = toNumber(ctrl.value);
  if (n === null) return { number: true };
  if (n < -90 || n > 90) return { range: true };
  return null;
}

export function lngValidator(ctrl: AbstractControl): ValidationErrors | null {
  const n = toNumber(ctrl.value);
  if (n === null) return { number: true };
  if (n < -180 || n > 180) return { range: true };
  return null;
}

/** Validación completa del payload (JSON) */
export function validateRoomPayload(payload: RoomCreatePayload): RoomCreateErrors {
  const errors: RoomCreateErrors = {};
  ...
  } else if (payload.imageUrls.some((u: string) => !isValidUrl(u))) {
  ...
  } else if (payload.serviceIds.some((id: number) => !Number.isInteger(id) || id <= 0)) {
...
export function hasErrors(errors: RoomCreateErrors): boolean {
