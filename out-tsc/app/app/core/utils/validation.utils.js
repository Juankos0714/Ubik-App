 ...
 * @returns error message if invalid, null if valid
 */
export function validateFileUpload(file, fieldName) {
    if (!file) {
        return `${fieldName} es requerido`;
    }
 ...
 */
export function collectValidationErrors(validations, stopOnFirstError = false) {
    const errors = [];
 ...
 */
export function toValidatorFn(fn, errorKey) {
    return (control) => {
